...
stock_basic["ts_code"] = stock_basic["ts_code"].astype("string").str.strip()
stock_basic["industry"] = stock_basic["industry"].astype("string").str.strip()
stock_basic = (
...
STOCK_INDUSTRIES = stock_basic.rename(
    columns={"ts_code": "code"}
).assign(
    sector=lambda frame: (
        frame["industry"].map(INDUSTRY_TO_SECTOR).fillna("工业")
    )
)
CODE_TO_INDUSTRY = dict(
    zip(
        STOCK_INDUSTRIES["code"],
        STOCK_INDUSTRIES["sector"],
        strict=True,
    )
)
