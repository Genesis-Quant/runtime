...
def run_backtest(
    ...
    *,
    ...
    callback_script, callback_names = _prepare_callbacks(callbacks)
    ...
    try:
        ...
        if overlap := set(output_columns) & SYSTEM_COLUMNS:
            raise ValueError(
                "symbol 和 tradeTime 由回测框架生成，DSL 不能重复定义："
                f"{sorted(overlap)}"
            )
        ...
        extension_columns = [
            column for column in output_columns if column not in ("time", "code")
        ]
        message_columns = [
            "symbol",
            "tradeTime",
        ...
        current_session.run(
            """
            coreBacktestMsg = coreDslOutput.copy()
            rename!(
                coreBacktestMsg,
                `code`time,
                `symbol`tradeTime
