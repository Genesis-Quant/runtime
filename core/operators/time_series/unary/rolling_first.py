...
class TimeSeriesUnaryRollingFirstParams(StrictModel):
    """unary.rolling_first 参数。"""
    ...
    def validate_min_periods(self) -> "TimeSeriesUnaryRollingFirstParams":
        """确保最少观测数不超过窗口。"""
...
class TimeSeriesUnaryRollingFirstOperator(TimeSeriesOperator):
    """按股票执行 rolling_first。"""

    op: Literal['unary.rolling_first'] = Field(..., description='按股票执行 rolling_first。')
    fields: UnaryFields = Field(..., description="该算符严格定义的输入字段。")
    params: TimeSeriesUnaryRollingFirstParams = Field(
        default_factory=TimeSeriesUnaryRollingFirstParams,
