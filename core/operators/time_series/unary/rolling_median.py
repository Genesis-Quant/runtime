...
class TimeSeriesUnaryRollingMedianParams(StrictModel):
    """unary.rolling_median 参数。"""
    ...
    def validate_min_periods(self) -> "TimeSeriesUnaryRollingMedianParams":
        """确保最少观测数不超过窗口。"""
...
class TimeSeriesUnaryRollingMedianOperator(TimeSeriesOperator):
    """按股票执行 rolling_median。"""

    op: Literal['unary.rolling_median'] = Field(..., description='按股票执行 rolling_median。')
    fields: UnaryFields = Field(..., description="该算符严格定义的输入字段。")
    params: TimeSeriesUnaryRollingMedianParams = Field(
        default_factory=TimeSeriesUnaryRollingMedianParams,
