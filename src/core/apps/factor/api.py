...
SOURCE_REF = "coreFactorSource"
UNFILTERED_REF = "coreFactorUnfilteredData"
...
def _column_list(
    ...
    if isinstance(value, (str, bytes)):
        raise TypeError(f"{name} 必须是字符串序列，不能是单个字符串")
    return list(value)
...
def analyze_factors(
    ...
    parameters = schema.FactorAnalysisParameters.model_validate(
        {
            ...
            "factor_columns": _column_list(
                factor_columns,
                "factor_columns",
            ),
            "return_columns": _column_list(
                return_columns,
                "return_columns",
            ),
            "n_groups": n_groups,
    ...
    try:
        ...
        logger.info("session.run: 加载 factor 模块并生成分析输入表")
        current_session.run(
            f"""
            use factor

            {INPUT_REF} = project_factor_output(
                {FILTERED_REF},
            ...
            if ({INPUT_REF}.rows() == 0) {{
                throw "factor analysis input is empty"
            }}
            """
        )
        ...
        for index, factor in enumerate(parameters.factor_columns):
            ...
            logger.info(
                f"session.run: 计算因子 {factor} 的 IC 和分组收益"
            )
            current_session.run(
                f"""
                {ic_ref} =
        ...
        logger.exception(
            f"因子分析失败：factor={list(parameters.factor_columns)}"
        )
        if owns_session:
